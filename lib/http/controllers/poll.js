...
// JSON API: Get similar polls
exports.similar = asyncHandler(async (req, res, next) => {
  ...
  // If no tags are present, get polls associated with owner
  if (!tags.length) {
    let poll

    try {
      poll = await Poll.get(id, 'owner')
    } catch (err) {
      req.log.error(err)
    }
  } else {
    ...
    try {
      pids = await Tag.findSimilar(tags)
    } catch (err) {
      req.log.error(err)
    }
  }
...
exports.render = asyncHandler(async (req, res, next) => {
  ...
  try {
    poll = await Poll.get(id, ['question'])
  } catch (err) {
    req.log.error(err)
    return next(err)
  }
