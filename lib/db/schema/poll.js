...
module.exports = class Poll extends Schema {
  ...
  async create (input = {}) {
    ...
    const hashid = await this.encode(id)
  ...
  async getTrending (polls, max = 3) {
    const hackerHotScore = hackerHot()
    return polls.map((poll) => {
      const score = poll.likes + poll.votes + poll.views
      return Object.assign({}, poll, {
        score: hackerHotScore(score, new Date(poll.created))
      })
  ...
  async sample (uid, offset=0, limit=10) {
    assert(isNumber(uid), 'invalid input type: expected number')

    const rows = await this._db.select(
      'id', 'status', 'created', 'expiry', 'question', 'public', 'permission', 'likes'
    )
  ...
  async encode (id) {
    assert(isNumber(id, 'input is not a number'))

    const result = await asyncify((input) => {
      const encoded = this._hash.encode(input)

      if (!encoded || !encoded.length) {
        throw new Error('encode: unable to encode value')
      }
  ...
  async decode (hash) {
    ...
    const result = await asyncify((input) => {
      ...
      if (!decoded || !decoded.length) {
        throw new Error('decode: unable to decode value')
      }
