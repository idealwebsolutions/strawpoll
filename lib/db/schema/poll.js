...
module.exports = class Poll extends Schema {
  ...
  async getTrending (max = 100) {
    ...
    const merged = results.map(async (result) => {  
      const views = await this._db.count('id')
        .from(VIEWS_TABLE_NAME)
        .where('id', result.id)
      
      const votes = await this._db.count('id')
        .from(VOTES_TABLE_NAME)
        .where('id', result.id)

      return Object.assign({}, result, {
        views: views[0]['count(`id`)'] || views[0]['count'],
        votes: votes[0]['count(`id`)'] || votes[0]['count'],
        score: hackerHotScore(result.likes, new Date(result.created))
      })
    })
  }

  async getNewest (max = 3) {
    const results = await this._db.select(['id', 'question', 'likes', 'created'])
      .from(POLLS_TABLE_NAME)
